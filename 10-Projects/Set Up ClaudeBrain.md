...
- [x] **Review the 60 summaries written on 2026-07-27** — done same day. Extracted the exact 60 from the commit that added them, then tested each: none merely restates its title, 59 are still live (only [[Strategy Promotion Ladder]] was superseded, correctly, in #21), and every substantive claim spot-checked traces to its own note's body. **No errors found.** Two summaries import a true claim from a *sibling* note rather than their own — [[Obsidian]] says "no cloud lock-in" where the note says "no lock-in", and [[Garry Tan]]'s states the vault's stance on GBrain rather than anything about him. Both accurate; left as-is rather than churned. Chris may still want to skim them — 60 of these are an agent's judgment about his notes.
- [x] **Fact-check the COMMAND reference notes** — done 2026-07-27 against `openclaw-ui` @ `ebda240`. Nine claims verified against source (the three `trading-safety.ts` caps, the 16 order-seal gates, the three promotion thresholds, Kalshi's daily-loss and one-loss-breaker), one error found and fixed: the Kalshi per-trade cap is **$2**, not $3 — `maxTradeUsd: 2`, with `$3` surviving only in a stale trailing comment. The [[Config Lies, Code Wins]] example also re-confirmed: `BRAIN_MAP.md` and `TRADING-SYSTEM.md` still say `$10,000` max order where source says `20_000`.
