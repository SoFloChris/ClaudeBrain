...
- (2026-07-29) **Incident: enabling SSH on the MSI desktop changed its Windows login password and locked Chris out.** Done by a session running locally on the MSI — no web session can reach that machine, and nothing about it is recorded in this repo. The cost was pure loss: SSH was never going to deliver phone-to-desktop control anyway, since a home machine behind NAT needs port forwarding to accept an inbound connection, while [[Directing Claude Code From Your Phone|Dispatch and Remote Control]] need no inbound access at all. **Lesson: never change a machine's login credentials in service of a remote-access plan — verify the plan can work first.** Recovery order that matters: pull the BitLocker recovery key from `account.microsoft.com/devices/recoverykey` *before* attempting anything, because a wrong turn without it is the one unrecoverable outcome.
